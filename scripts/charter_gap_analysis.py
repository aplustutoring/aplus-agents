#!/usr/bin/env python3
"""Charter renewal gap analysis — READ-ONLY against HubSpot + Teachworks.

Finds charter families with a 25/26 deal but no 26/27 renewal, enriched with
Teachworks invoice history, and writes a 4-tab xlsx:
...
2026-06-01 OR has "26/27" in the dealname; otherwise it is a GAP family.
...
Usage:
  python3 scripts/charter_gap_analysis.py [--out PATH]
...
def main():
    ...
    hdr1 = ["Last", "First", "Email", "Phone", "Last Invoice", "Total Invoiced",
            "Invoices", "Months Since Last Invoice", "Segment", "HubSpot"]
    rows1 = []
    for fam, f in sorted(gap_with_inv, key=lambda x: -x[1]["total_invoiced"]):
        ...
        rows1.append([c["last"], c["first"], c["email"], c["phone"],
                      ...
                      seg, HUBSPOT_RECORD_URL.format(id=c["id"])])
