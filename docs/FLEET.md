...
**53 registered agents** — 37 active · 12 manual · 4 deprecated · across 13 engines.
...
**Reports, drafts, or waits for a human (16):** `lead-agent`, `topic-gen`, `blog-metrics`, `deal-sync-relay`, `call-agent-webhook-relay`, `feedback-agent`, `task-completion-sweep`, `email-weekly-digest`, `email-daily-summary`, `email-hourly-update`, `email-po-daily-report`, `email-draft-feedback`, `credential-expiry`, `fleet-docs`, `pr-merge-nudge`, `branch-hygiene`.
...
| Agent | Runs | Status | Reads | Writes |
|---|---|---|---|---|
| **lead-agent**<br>Lead agent — reason over new leads | 08:00-18:30 PT | active (draft) | HubSpot:contacts, HubSpot:deals, HubSpot:calls, HubSpot:meetings, JustCall, Anthropic | HubSpot:tasks, HubSpot:notes, HubSpot:contacts, ops/lead_agent/state/ |
| **lead-intake**<br>Lead intake — stage 01 first touch | 07:00-16:59 PT | deprecated (draft) | HubSpot:contacts, HubSpot:deals, JustCall, ops/lead_intake/state/cursor.json | HubSpot:tasks, HubSpot:notes, ops/lead_intake/state/ |
